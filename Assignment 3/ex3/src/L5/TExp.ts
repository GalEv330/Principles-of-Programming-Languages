...
import { all, append, chain, concat, equals, map, sort, uniq, zip, filter, includes, is, reduce } from "ramda";
...
export type CompoundTExp = extendedProcTexp | TupleTExp | UnionTExp | InterTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x) || isExtendedProcTexp(x);

export type NonTupleTExp = AtomicTExp | extendedProcTexp | TVar | UnionTExp | InterTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isExtendedProcTexp(x) || isTVar(x) || isUnionTExp(x) || isInterTExp(x);

export type extendedProcTexp = typePredTExp | ProcTExp;
export const isExtendedProcTexp = (x: any): x is extendedProcTexp => isTypePredTExp(x) || isProcTExp(x);
...
export type typePredTExp = { tag: "typePredTExp"; param: TExp };
export const makeTypePredTExp = (param: TExp ): typePredTExp =>
    ({ tag: "typePredTExp", param: param});
...
const flattenSortInter = (tes: TExp[]): TExp[] => {
    const sorted = sort(superTypeComparator, flattenInter(tes));
    return checkIfeveryElementIsAny(sorted) ? [makeAnyTExp()] : removeDuplicatesAndAny(sorted);
}
    
const checkIfeveryElementIsAny = (tes: TExp[]): boolean =>
    reduce((acc, x) => (acc && isContainAllTExp(x)), true, tes);

const isContainAllTExp = (te: TExp): boolean =>
    isAnyTExp(te) ? true :
    (isUnionTExp(te) || isInterTExp(te)) ? containsType(te.components, makeAnyTExp()) 
    || (containsType(te.components, makeNumTExp()) && containsType(te.components, makeBoolTExp()) && containsType(te.components, makeStrTExp())) : false;    
...
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp => {
    if (!isCompoundTExp(te1) && !isCompoundTExp(te2)) return te1;
    if (!isCompoundTExp(te1)) {
        if (superTypeComparator(te1, te2) == 1) return makeNeverTExp();
        return te1;
    }
    if (superTypeComparator(te1, te2) == 0) return makeNeverTExp();
    const toRemove = makeRemove(te1, te2);
    return (makeUnionTExp(toRemove));
}
    
const makeRemove = (te1: TExp, te2: TExp): TExp[] =>
    isUnionTExp(te1) ? te1.components.filter((x) => !isSubType(x, te2)) : [te1];
...
// SubType comparator (check if te1 is a subtype of te2)
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    (isTypePredTExp(te1)) ? isSubType(makeBoolTExp(), te2) :
    (isTypePredTExp(te2)) ? isSubType(te1, makeBoolTExp()) :    
    
    isAnyTExp(te2) ? true :
    isNeverTExp(te1) ? true :
    isNeverTExp(te2) ? false :
    isAnyTExp(te1) ? isContainAllTExp(te2) :

    (isUnionTExp(te2) && isUnionTExp(te1)) ? isSubset(te1.components, te2.components) :
    (isUnionTExp(te2) && isInterTExp(te1)) ? isSubset(filter((x) => isSubType(x, te1), te1.components), te2.components) :
    (isInterTExp(te2) && isUnionTExp(te1)) ? isSubset(te1.components, filter((x) => isSubType(x, te2), te2.components)) :
    (isInterTExp(te2) && isInterTExp(te1)) ? te2.components.every(t2Comp => findCommonSubtypes(te1.components).some(t1Comp => isSubType(t1Comp, t2Comp))) :

    isUnionTExp(te2) ? containsType(te2.components, te1) :
    isInterTExp(te2) ? reduce((acc, x) => (acc && isSubType(te1, x)), true, te2.components) :
    
    isUnionTExp(te1) ? te1.components.length === 1 ? equals(te1.components[0], te2) : false :
    isInterTExp(te1) ? filter((x) => isSubType(x, te2), te1.components).length === 1 ? equals(filter((x) => isSubType(x, te2), te1.components)[0], te2) : false :
...
// Assuming isSubType(x, y) checks if x is a subtype of y
function findCommonSubtypes(components: TExp[]): TExp[] {
    return components.filter(x => 
      components.every(y => isSubType(x, y))
    );
  }
...
const parseTypePredTExp = (texps: Sexp[]): Result<TExp> => {
    const lastIndex = texps.length - 1;
    const pos1 = texps.indexOf('->');
    const pos2 = texps.indexOf('is?');

    return texps.length !== 4 ? makeFailure(`Invalid number of elements in the type predicate - ${format(texps)}`) :
           texps[0] !== "any" ? makeFailure(`First element in type predicate must be 'any' - ${format(texps)}`) :
        texps[lastIndex - 1] !== 'is?' ? makeFailure(`Worng place for 'is?' in the type predicate - ${format(texps)}`) :
        pos1 === -1 ? makeFailure(`'->' is missing in the type predicate - ${format(texps)}`) :
        pos2 === -1 ? makeFailure(`'is?' is missing in the type predicate - ${format(texps)}`) :
        texps[lastIndex - 2] !== '->' ? makeFailure(`'->' is placed incorrectly in the type predicate - ${format(texps)}`) :
                                        bind(parseTExp(texps[lastIndex]), (comp: TExp) => makeOk(makeTypePredTExp(comp)));
};
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isInterTExp(x)?bind(unparseTExp(x.components[0]), (exp1: string) =>
                                makeOk(`(inter ${exp1} )`)) :
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isTypePredTExp(x) ? bind(unparseTExp(x.param), (comp: string) =>
                                makeOk(`any -> (is? ${comp})`)) :
