...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, parseL5Program, VarRef} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp, makeUnionTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType, isExtendedProcTexp, isTypePredTExp, makeTypePredTExp, typePredTExp, makeDiffTExp, 
         isAnyTExp,
         isBoolTExp} from "./TExp";
import { isEmpty, allT, first, rest, NonEmptyList, List, isNonEmptyList } from '../shared/list';
import { Result, makeFailure, bind, makeOk, zipWithResult, either, mapResult } from '../shared/result';
...
export const checkCompatibleType = (te1: TExp, te2: TExp, exp: Exp): Result<true> =>
  ...
  isAnyTExp(te1) ? makeOk(true) : isAnyTExp(te2) ? makeOk(true) :
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<[TEnv, TEnv]> => {
    if (!isAppExp(e)) return makeFailure(`not an application expression`);
    if (!isVarRef(e.rator)) return makeFailure(`Rator is not a variable reference`);
    if (e.rands.length !== 1 || !isVarRef(e.rands[0])) return makeFailure(`Operand is not a variable reference`);

    const rator = e.rator.var;
    const operand = e.rands[0].var;

    return bind(typeofApp(e, tenv),(te: TExp) => 
                bind(applyTEnv(tenv, rator), (te: TExp) =>
                    isTypePredTExp(te) ? (makeOk([makeExtendTEnv([operand], [te.param], tenv), // env for then
                                        makeExtendTEnv([operand], [makeDiffTExp(te, te)], tenv)])) : // env for alt
            makeFailure(`Not a type predicate`)))
}


export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    const testTE = isTypePredApp(ifExp.test, tenv);
    return either(
        bind(testTE, (extendedTEnv: [TEnv, TEnv]) => {
            const thenTE = typeofExp(ifExp.then, extendedTEnv[0]);
            const altTE = typeofExp(ifExp.alt, extendedTEnv[1]);
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    const constraint1 = isTypePredTExp(proc.returnTE) ? bind(typeofExps(proc.body, extTEnv), (body: TExp) => isBoolTExp(body) ? 
    makeOk(true) : bind(typeofExps(proc.body, extTEnv), (body: TExp) => checkCompatibleType(body, proc.returnTE, proc))) : 
    bind(typeofExps(proc.body, extTEnv), (body: TExp) => 
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (rator: TExp) => 
        !isExtendedProcTexp(rator) ? makeFailure(`Application of non-procedure: ${unparseTExp(rator)}`) :
        isTypePredTExp(rator) ? makeOk(makeTypePredTExp(rator)) :
        app.rands.length === rator.paramTEs.length ? bind((zipWithResult((rand, trand) => bind(typeofExp(rand, tenv), (typeOfRand: TExp) => 
                            checkCompatibleType(typeOfRand, trand, app)), app.rands, rator.paramTEs)), _ => makeOk(rator.returnTE)) :
        makeFailure(`Procedure needs ${rator.paramTEs.length} arguments, but got ${app.rands.length}`));
