...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isClassExp(x) || isLitExp(x) || isLetExp(x);
...
// Function to parse 'class' special form (Question 2)
const parseClassExp = (params: Sexp[]): Result<ClassExp> => {
    if (!isArray(params) || params.length !== 2) {
        return makeFailure("Invalid number of arguments for 'class'");
    }
    if (!isGoodBindings(params[1])) {
        return makeFailure('Malformed bindings in "class" expression');
    }
    if(!(isArray(params[0]) && allT(isString, params[0]))){
        return makeFailure("Invalid number of vars for 'class'");
    }

    const acts = map(b => b[0], params[1]);
    const actsResult = mapResult(parseL3CExp, map(second, params[1]));
    const methods = mapv(actsResult, (vals: CExp[]) => zipWith(makeBinding, acts, vals));

    const fields = map(makeVarDecl, params[0])
    if(!isOk(methods)){
        return makeFailure("Invalid methods for 'class'");
    }
    return makeOk(makeClassExp(fields, methods.value));
};
...
// Function to unparse 'class' special form (Question 2)
