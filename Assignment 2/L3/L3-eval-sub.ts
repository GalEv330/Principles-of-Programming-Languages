...
import { isClosure, makeClosure, Closure, Value, isClass, makeClass, makeObject, Class, isObject, isSymbolSExp, Object, closureToString, valueToString } from "./L3-value";
...
const applyClass = (proc: Class, args: Value[], env: Env): Result<Value> =>
    proc.fields.length !== args.length ? makeFailure("wrong number of arguments") :
...
const applyObject = (proc: Object, args: Value[], env: Env): Result<Value> => {
    ...
    const method = proc.class.methods[methodIndex];
    if (!isProcExp(method.val)) return makeFailure("method must be a procedure");
    if (method.val.args.length !==args.length-1) return makeFailure("wrong number of arguments");
    const vars = (method.val.body, proc.class.fields.map((f) => f.var));
    const values : CExp[] = map(valueToLitExp, proc.fieldValues);
    const closure = makeClosure(method.val.args, substitute(method.val.body, vars, values));
    return applyClosure(closure, tail(args), env);
  };
